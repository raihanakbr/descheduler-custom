...
export const options = {
  scenarios: {
    mixed: {
      ...
      stages: [
        { duration: __ENV.RAMP_UP || '30s', target: Number(__ENV.TARGET_RPS || 10) },
        { duration: __ENV.HOLD || '90s', target: Number(__ENV.TARGET_RPS || 30) },
...
export default function () {
  ...
  if (r < 0.45) {
    ...
    ip = workerIPs[1];
    path = '/memory/work?cpu_ms=20&mem_mb=64&hold_ms=1000';
  } else {
    ip = workerIPs[2];
