...
def main():
    ...
    for system in ("N0", "R0", "R1", "H0", "H1"):
        ...
        p95_before = [nested(run, "api_load", "before", "p95_ms") for run in runs]
        p95_after = [nested(run, "api_load", "after", "p95_ms") for run in runs]
        p95_delta = [
            after - before
        ...
        output["systems"][system] = {
            ...
            "failure_rate_after": describe([
                nested(run, "api_load", "after", "failure_rate") for run in runs
            ]),
