...
OUTPUT_DIR="results/walkthrough/r0-no-load/$TIMESTAMP"
mkdir -p "$OUTPUT_DIR"
...
log() {
  printf '[walkthrough-r0-no-load] %s %s\n' "$(date +%H:%M:%S)" "$*"
}
...
log "step 6/7: run descheduler R0 (no ActualUsageEvictor)"
...
grep -E "(Evicted pod|Eviction decision|pod selected|Processing node)" "$OUTPUT_DIR/descheduler.log" || true
