...
# Three dense destination nodes. Each exceeds the HNU threshold on CPU.
for i in 0 1 2; do
  node="${workers[$i]}"
  deploy_workload "workload-destination-$((i + 1))" "$node" 2 650m 150Mi 700m
done

# Two idle, deliberately underutilized sources.
deploy_workload workload-idle-source-1 "${workers[3]}" 1 50m 160Mi 1000m
deploy_workload workload-idle-source-2 "${workers[4]}" 1 50m 160Mi 1000m

# The only busy pod is isolated on the third underutilized source.
deploy_workload workload-api "${workers[5]}" 1 50m 240Mi 1000m

for node in "${workers[@]}"; do
  kubectl uncordon "$node" >/dev/null
done

kubectl apply -f "$HNU_ROOT/k8s/services.yaml"
...
python3 "$HNU_ROOT/scripts/validate-layout.py" \
