...
SYSTEM="${1:?usage: run-with-load.sh <H0|H1>}"
case "$SYSTEM" in H0|H1) ;; *) echo "invalid system: $SYSTEM" >&2; exit 2 ;; esac

HNU_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
PARENT_ROOT="$(cd "$HNU_ROOT/.." && pwd)"
NS="actual-usage-exp"
TIMESTAMP="$(date +%Y%m%d-%H%M%S)"
OUTPUT_DIR="$HNU_ROOT/results/${SYSTEM,,}-load/$TIMESTAMP"
mkdir -p "$OUTPUT_DIR"
...
if [[ "$SYSTEM" == "H0" ]]; then
  policy="$HNU_ROOT/policies/h0-hnu.yaml"
else
  policy="$HNU_ROOT/policies/h1-hnu-actual.yaml"
fi
