...
func appendNodeCSV(path string, snapshot Snapshot) error {
	...
	defer w.Flush()
	if newFile {
		if err := w.Write([]string{"timestamp", "node", "metrics_source", "cpu_used_milli", "memory_used_bytes", "cpu_capacity_milli", "memory_capacity_bytes", "cpu_usage_ratio", "memory_usage_ratio", "rii", "fragmented"}); err != nil {
			return err
		}
	}
